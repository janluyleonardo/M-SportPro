            ...
            <div>
                ...
                <p class="text-xs text-gray-500 font-bold tracking-wide mt-0.5">
                    @if(auth()->user()->club_id)
                        {{ auth()->user()->club->name ?? 'Mi Club' }} &bull; {{ \App\Models\Student::where('club_id', auth()->user()->club_id)->count() }} {{ __('total_students_count') }}
                    @else
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            @if(isset($error_message))
                <div class="bg-red-50 border-2 border-red-100 p-6 rounded-3xl flex items-center space-x-4 animate-shake">
                    ...
                    <div>
                        <h4 class="text-red-900 font-black uppercase text-xs tracking-widest mb-1">{{ __('Limited Access') }}</h4>
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                <form action="{{ route('payments.index') }}" method="GET" class="relative">
                    <input type="text" name="search" value="{{ $search }}" placeholder="{{ __('Search by student name or document...') }}" class="w-full pl-12 pr-4 py-3 border-gray-200 rounded-2xl focus:ring-club-primary focus:border-club-primary text-sm shadow-inner bg-gray-50/50">
