    ...
    <div class="py-8">
        ...
        {{-- El layout principal ya muestra las alertas de session('success') y errores --}}
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                ...
                <form action="{{ route('locations.store') }}" method="POST" class="flex items-end gap-4">
                    @csrf
                    <div class="flex-1">
                    ...
                    <button type="submit"
                            class="px-6 py-3 bg-club-primary text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:opacity-90 transition-all shadow-md whitespace-nowrap">
                        ...
                        <div class="flex items-center justify-between px-6 py-4 hover:bg-gray-50/50 transition-colors">
                            <div class="flex items-center space-x-4">
                                <div class="w-10 h-10 rounded-xl flex items-center justify-center
                                    {{ $location->active ? 'bg-club-primary text-white' : 'bg-gray-100 text-gray-400' }}">
                                    <i class="bi bi-geo-alt-fill text-sm"></i>
                                </div>
                                <div>
                                    <h4 class="font-black text-gray-900 text-sm">{{ $location->name }}</h4>
                                    <p class="text-[10px] text-gray-400 font-medium">{{ $location->description ?: 'Sin descripción' }}</p>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center space-x-3">
                                ...
                                <form action="{{ route('locations.update', $location) }}" method="POST">
                                    ...
                                    <button type="submit" title="{{ $location->active ? 'Desactivar' : 'Activar' }}"
                                            ...
                                            {{ $location->active ? 'bg-yellow-50 text-yellow-600 hover:bg-yellow-100' : 'bg-green-50 text-green-600 hover:bg-green-100' }}">
                                        <i class="bi {{ $location->active ? 'bi-pause-circle-fill' : 'bi-play-circle-fill' }} text-base"></i>
                                    </button>
                                </form>
                                ...
                                <form action="{{ route('locations.destroy', $location) }}" method="POST"
                                    ...
                                    @csrf @method('DELETE')
                                    <button type="submit" class="p-2 bg-red-50 text-red-400 rounded-xl hover:bg-red-100 hover:text-red-600 transition-all">
